...
stim(4).orient=-45; stim(5).type='grating';
stim(4).rad=2; stim(4).ap='gaussian';
stim(4).npix=frame.apSize;

%% build the array 

bigMatSize = [(frame.apNum+1)*frame.apSep] + [frame.apSize*frame.apNum];
...
for i =1:frame.apNum
    for j=1:frame.apNum
        ...
        [ sw2D] = putinapeture(sw2D, stim((i-1)*2+j));
        sw2D = ((sw2D+1)*127.5)+1;
